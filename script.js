...
const PROPERTY_TEMPLATES = [
  { id: "starter-bungalow", name: "Starter Bungalow", price: 500, grossRent: 25, upkeep: 3, cycleTime: 10, managerCost: 750, managerSalary: 2, map: { x: "22%", y: "21%" } },
  { id: "oak-street-cottage", name: "Oak Street Cottage", price: 1250, grossRent: 70, upkeep: 9, cycleTime: 15, managerCost: 1800, managerSalary: 5, map: { x: "61%", y: "20%" } },
  { id: "pine-duplex", name: "Pine Duplex", price: 3000, grossRent: 180, upkeep: 25, cycleTime: 25, managerCost: 4500, managerSalary: 15, map: { x: "36%", y: "38%" } },
  { id: "brick-ranch", name: "Brick Ranch", price: 7500, grossRent: 475, upkeep: 70, cycleTime: 40, managerCost: 10000, managerSalary: 40, map: { x: "78%", y: "42%" } },
  { id: "downtown-townhome", name: "Downtown Townhome", price: 18000, grossRent: 1250, upkeep: 210, cycleTime: 60, managerCost: 25000, managerSalary: 125, map: { x: "18%", y: "61%" } },
  { id: "lakeside-rental", name: "Lakeside Rental", price: 45000, grossRent: 3400, upkeep: 650, cycleTime: 90, managerCost: 60000, managerSalary: 300, map: { x: "59%", y: "64%" } },
  { id: "maple-fourplex", name: "Maple Fourplex", price: 110000, grossRent: 8500, upkeep: 1800, cycleTime: 120, managerCost: 150000, managerSalary: 800, map: { x: "34%", y: "84%" } },
  { id: "small-apartment-building", name: "Small Apartment Building", price: 275000, grossRent: 24000, upkeep: 5500, cycleTime: 180, managerCost: 375000, managerSalary: 2500, map: { x: "77%", y: "82%" } },
];
...
function renderChrome() {
  ...
  dom.managerUnlockNote.textContent = managersUnlocked()
    ? "Property managers are unlocked."
...
function renderProperties() {
  dom.propertiesList.innerHTML = state.properties.map((property) => {
    ...
    const managerLine = property.owned
      ? `<div class="metric"><span>Manager</span><strong>${property.managerHired ? `Hired (${formatMoney(property.managerSalary)}/cycle)` : managersUnlocked() ? `${formatMoney(property.managerCost)}` : "Locked"}</strong></div>`
      : "";
    const actions = property.owned
      ? `<button class="secondary-action" data-action="view-map" data-id="${property.id}" type="button">View on Map</button>
         ${managerButtonMarkup(property)}`
      ...
      <article class="property-card">
        <div class="property-top">
          <div>
            ...
            <p class="muted">${property.cycleTime}s rent cycle</p>
          </div>
        ...
        <div class="money-grid">
          ...
          <div class="metric"><span>Upkeep</span><strong>${formatMoney(property.upkeep)}</strong></div>
          <div class="metric"><span>Net rent</span><strong>${formatMoney(property.netRent)}</strong></div>
          ${managerLine}
        </div>
...
function managerButtonMarkup(property) {
  ...
  const disabled = state.cash < property.managerCost ? "disabled" : "";
  return `<button data-action="hire-manager" data-id="${property.id}" ${disabled} type="button">Hire Manager ${formatMoney(property.managerCost)}</button>`;
}
...
function renderMap() {
  ...
  dom.mapGrid.innerHTML = owned.map((property) => {
    const progressPercent = Math.min(100, Math.round((property.progress / property.cycleTime) * 100));
...
function renderMenu() {
  ...
  const stats = [
    ...
    ["Manager salaries", formatMoney(state.stats.lifetimeManagerSalaries)],
...
function collectRent(id, automatic = false, sourceElement = null) {
  ...
  const managerSalary = automatic ? property.managerSalary : 0;
  const net = property.grossRent - property.upkeep - managerSalary;
  state.cash += net;
  state.stats.lifetimeGrossRent += property.grossRent;
  state.stats.lifetimeUpkeep += property.upkeep;
  state.stats.lifetimeManagerSalaries += managerSalary;
...
function hireManager(id) {
  ...
  if (state.cash < property.managerCost) return;
  state.cash -= property.managerCost;
...
function update(dt) {
  ...
  ownedProperties().forEach((property) => {
    ...
    if (property.managerHired) {
      while (property.progress >= property.cycleTime) {
        property.progress -= property.cycleTime;
        collectRent(property.id, true);
      }
      property.readyToCollect = false;
      return;
    }
    if (property.progress >= property.cycleTime) {
      property.progress = property.cycleTime;
  ...
  try {
    ...
    fresh.properties = fresh.properties.map((property) => {
      ...
      return {
        ...
        progress: clampNumber(savedProperty.progress, 0, property.cycleTime),
...
function renderGameToText() {
  const payload = {
    ...
    stats: {
      ...
      managerSalaries: Math.round(state.stats.lifetimeManagerSalaries),
    ...
    visibleProperties: state.properties.map((property) => ({
      ...
      name: property.name,
      owned: property.owned,
      progressPercent: Math.round((property.progress / property.cycleTime) * 100),
      readyToCollect: property.readyToCollect,
      managerHired: property.managerHired,
    })),
